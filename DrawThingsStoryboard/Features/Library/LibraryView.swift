...
struct LibraryBrowserView: View {
    ...
    private var customers: [MockCustomer] { studio?.customers ?? [] }
    private var customer: MockCustomer? {
        guard customers.indices.contains(customerIndex) else { return nil }
        return customers[customerIndex]
    }
    private var episodes: [MockEpisode] { customer?.episodes ?? [] }
    ...
    var body: some View {
                ...
                VStack(spacing: 0) {
                    ...
                    if let studio {
                        LibraryLevelHeader(
                            ...
                            onForward: { studioIndex += 1; customerIndex = 0; selectedItem = nil }
                        )
                        LibraryCastGrid(
                            characters: studio.characters, locations: studio.locations,
                            selectedItem: $selectedItem, emptyText: "No studio-level assets."
                        )
                    }
                    ...
                    if let customer {
                        LibraryLevelHeader(
                            ...
                            onForward: { customerIndex += 1; selectedItem = nil }
                        )
                        ...
                        LibraryCastGrid(
                            characters: custChars, locations: custLocs,
                            selectedItem: $selectedItem, emptyText: "No customer-level assets."
                        )
                    } else {
                        Text("No customers in this studio.").font(.caption).foregroundStyle(.tertiary).padding(.vertical, 12)
                    }
                    ...
                    if episodes.isEmpty {
                        HStack {
                            Label("Episodes", systemImage: "film").font(.subheadline.weight(.medium)).foregroundStyle(.secondary)
                            Spacer()
                        }
                        .padding(.horizontal, 14).padding(.top, 10)
                        Text("No episodes for this customer.").font(.caption).foregroundStyle(.tertiary).padding(.vertical, 12)
                    } else {
                        ForEach(Array(episodes.enumerated()), id: \.element.id) { idx, episode in
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                LibraryCastGrid(
                                    ...
                                    selectedItem: $selectedItem, emptyText: "No assets in this episode."
                                )
                            }
        ...
        .onChange(of: studioIndex) { _, _ in
            if customerIndex >= customers.count { customerIndex = max(0, customers.count - 1) }
        }
    }
}
...
private struct LibraryCastGrid: View {
    let characters: [CastingItem]; let locations: [CastingItem]
    @Binding var selectedItem: CastingItem?
    ...
    var body: some View {
        if characters.isEmpty && locations.isEmpty {
            ...
            LazyVGrid(columns: columns, spacing: 12) {
                ForEach(locations) { item in libraryTile(item: item).onTapGesture { selectedItem = item } }
                ForEach(characters) { item in libraryTile(item: item).onTapGesture { selectedItem = item } }
            }
    ...
    private func libraryTile(item: CastingItem) -> some View {
        ...
        let deleteAction: (() -> Void)? = onDelete != nil ? { onDelete?(item.id) } : nil
        return UnifiedThumbnailView(
            ...
            badges: ThumbnailBadges(
                ...
                showDeleteButton: deleteAction != nil, onDelete: deleteAction,
