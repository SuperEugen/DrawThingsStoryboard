...
struct ProjectsDetailView: View {
    ...
    private var inheritedLook: (look: GenerationTemplate, source: ProjectsLevel)? {
        guard let si = studioIndex else { return nil }
    ...
    private var resolvedLook: (look: GenerationTemplate, source: ProjectsLevel)? {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                // Type badge header
                ProjectsTypeHeader(level: level, color: themeColor)
...
// MARK: - Projects type header

private struct ProjectsTypeHeader: View {
    let level: ProjectsLevel
    let color: Color
...
private struct PreferredLookSection: View {
    let level: ProjectsLevel
    let resolvedLook: (look: GenerationTemplate, source: ProjectsLevel)?
    let hasOwnLook: Bool
    let onRemove: () -> Void
    let onAdd: () -> Void
